...
if [ -f "$(brew --prefix)/opt/bash-git-prompt/share/gitprompt.sh" ]; then
  __GIT_PROMPT_DIR_BASH=$(brew --prefix)/opt/bash-git-prompt/share
...
- If you cloned the repo to a directory other then ~/.bash-git-prompt , set __GIT_PROMPT_DIR_BASH in ~/.config/fish/config.fish
   to that path
