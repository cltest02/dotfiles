...
Edit your modified config `~/.bashrc` file in order to customize Bash-it, set `REDPILL_THEME` to the theme name `atomic`.

Examples:

```bash
# Use the "atomic" theme
export REDPILL_THEME="atomic"
```
...
sed -i 's/'"$REDPILL_THEME"'/atomic/g' ~/.bashrc
```
Command to execute for macOS:
```bash
# Set the "atomic" theme replacing the theme you are using of bash-it
sed -i '' 's/'"$REDPILL_THEME"'/atomic/g' ~/.bash_profile
```
